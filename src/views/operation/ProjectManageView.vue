  ...
  <div class="page-stack">
      ...
      <div class="mb-4 flex flex-wrap items-end gap-3">
        <form class="contents" @submit.prevent="search">
          <label class="flex w-44 shrink-0 flex-col gap-1 text-sm">
            <span class="text-gray-500">{{ t('operation.project.projectName') }}</span>
            <input v-model="query.projectName" type="text" class="field-input" />
          </label>
          <label class="flex w-44 shrink-0 flex-col gap-1 text-sm">
            <span class="text-gray-500">{{ t('operation.project.serverIp') }}</span>
    ...
    <AppModal :open="modalOpen" :title="modalTitle" wide @close="closeModal">
      <form class="grid grid-cols-1 gap-4 sm:grid-cols-2" @submit.prevent="submitForm">
        <label class="flex flex-col gap-1 text-sm sm:col-span-2"><span class="text-gray-500">{{ t('operation.project.projectName') }} *</span><input v-model="form.projectName" class="field-input" /></label>
        <label class="flex flex-col gap-1 text-sm sm:col-span-2"><span class="text-gray-500">URL</span><input v-model="form.url" class="field-input" /></label>
        <label class="flex flex-col gap-1 text-sm"><span class="text-gray-500">{{ t('operation.project.serverIp') }}</span><input v-model="form.serverIp" class="field-input" /></label>
        <label class="flex flex-col gap-1 text-sm"><span class="text-gray-500">{{ t('operation.project.innerIp') }}</span><input v-model="form.innerIp" class="field-input" /></label>
        <label class="flex flex-col gap-1 text-sm"><span class="text-gray-500">{{ t('operation.project.port') }}</span><input v-model="form.port" class="field-input" /></label>
        <label class="flex flex-col gap-1 text-sm"><span class="text-gray-500">{{ t('operation.project.deployPath') }}</span><input v-model="form.deployPath" class="field-input" /></label>
