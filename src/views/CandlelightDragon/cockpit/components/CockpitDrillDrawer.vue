...
import type { DrillRow } from '@/types/cockpit'
...
<template>
  <AppModal :open="open" :title="title" wide @close="emit('close')">
    <nav v-if="breadcrumb.length" class="mb-4 flex flex-wrap items-center gap-1 text-sm text-gray-500 dark:text-gray-400">
      <template v-for="(crumb, i) in breadcrumb" :key="i">
        ...
        <span :class="i === breadcrumb.length - 1 ? 'font-medium text-brand-600 dark:text-brand-400' : ''">
          {{ crumb.startsWith('cockpit.') ? t(crumb) : crumb }}
        </span>
      </template>
    </nav>

    <div v-if="loading" class="py-12 text-center text-gray-500 dark:text-gray-400">{{ t('cockpit.loading') }}</div>
    <div v-else-if="!rows.length" class="py-12 text-center text-gray-500 dark:text-gray-400">{{ t('cockpit.drillEmpty') }}</div>
    ...
    <template #footer>
      <button type="button" class="btn-ghost" @click="emit('close')">{{ t('cockpit.close') }}</button>
    </template>
  </AppModal>
</template>
