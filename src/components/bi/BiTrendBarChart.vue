...
use([CanvasRenderer, EBarChart, GridComponent, TooltipComponent, MarkLineComponent])
...
const option = computed(() => {
  ...
  const hi = props.highlightIndex ?? -1
  const values = ready.value ? props.values : props.values.map(() => 0)
  const max = Math.max(...props.values, props.targetLine, 1) + 20
    ...
    series: [
      {
        ...
        data: values.map((value, i) => ({
          value,
          itemStyle: {
            color: i === hi ? '#6366f1' : mutedBar,
            borderRadius: [6, 6, 0, 0],
            decal: i === hi ? undefined : stripeDecal,
          },
        })),
        barWidth: 36,
