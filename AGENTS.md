...
| Tab1 投喂 Raw | T20a · `POST /kb/ingest/raw-upload` | T20a 未就绪：`VITE_MOCK_KB_IMPORT=true`；上线后 `false` |
| Tab3 成品导入 | T20b · `POST /kb/wiki/page/import` | 同上，与 Tab2 Mock 开关独立 |
...
- 无权限时 Tab1 **只读** + 提示（见 distribute `docs/api/kb-import-entry-frontend.md` §5.4）
