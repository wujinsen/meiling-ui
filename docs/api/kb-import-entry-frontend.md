...
> **读者**：meiling-ui 前端。  
> **状态**：**T20a/T20b 后端 ✅**；前端三 Tab **UI + 直联 ✅**（2026-07-10）。  
...
| 冲突策略 | 单选横排三列：`SKIP`（默认）/ `OVERWRITE` / `RENAME` |
...
| 2 | 选单个 `.md`；可选 zip 插图包（T20e） |
| 3 | slug / title（可编辑；slug 默认文件名 stem） |
| 4 | 预览落盘路径 `{dir_slug}/{slug}.md` |
| 5 | slug 冲突 FAIL/OVERWRITE · ☑ 导入后 Sync（**默认开**）· ☑ lint 预检 |
| 6 | 确认 → import → 右侧结果区 + `KbWorkflowNextSteps` |
...
### 6.3 请求 · `POST /kb/wiki/page/import`
...
| 日期 | 说明 |
|------|------|
| 2026-07-10 | Tab1/Tab3 UI 排版：冲突横排、成品导入单卡 2:1 布局、样式类说明 |
