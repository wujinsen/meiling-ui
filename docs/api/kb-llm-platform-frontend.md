...
平台管理员在 **企业知识库 → LLM 配置** 配置全站共用的 LLM（Ask / Ingest / Wiki 治理 / Express 一键入库）。
...
|----|-----|
| 菜单名 | LLM 配置 |
| 父菜单 | 企业知识库（`parent_id=900`） |
| 路由 path | `kb-llm`（完整 URL 如 `/knowledge/kb-llm`） |
| 组件 path | `knowledge/kb-llm/index` |
